...
#
# Environment:
#   RULES_DISTILL_GLOBAL_DIR   Override {{PIPELINE_ROOT}}/skills (for testing only;
...
get_mtime() {
  ...
  date -u -d "@$secs" +%Y-%m-%dT%H:%M:%SZ 2>/dev/null ||   # portability-ok: date-flags — BSD falls through to -r
