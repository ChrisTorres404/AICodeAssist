...
- Run AFTER `the `bug-triage` skill` (which finds the other 54 bug types)
- Run BEFORE `/verify` (which verifies fixes work)
- Feeds into `the `test-first` skill` — every bug found here should get a test
