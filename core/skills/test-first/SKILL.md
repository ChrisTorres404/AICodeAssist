---
name: test-first
description: Use this skill when writing new features, fixing bugs, or refactoring code. Enforces test-driven development with executed behavioural evidence (see the testing rule) including unit, integration, and E2E tests.
...
- Continuing from a the `planner` role output or another `*.plan.md` implementation plan
...
- Minimum executed behavioural evidence (see the testing rule) (unit + integration + E2E)
...
1. **Run the stack detector** (ships with this pipeline):

   ```bash
   {{PIPELINE_ROOT}}/bin/detect-stack . --json
   ```

   Its `package_manager` field resolves npm / pnpm / yarn / bun from the `package.json` `packageManager` field and the lockfile; its `commands` field carries the test and build commands it found.
...
```bash
<coverage>
# Verify executed behavioural evidence (see the testing rule) achieved
```
