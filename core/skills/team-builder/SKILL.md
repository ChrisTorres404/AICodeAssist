...
1. **`claude agents` command** (primary) — run `claude agents` to get all agents known to the CLI, including user agents, plugin agents (e.g. `architect`), and built-in agents. This automatically covers this pipeline marketplace installs without any path configuration.
2. **File glob** (fallback, for reading agent content) — agent markdown files are read from:
   ...
   - `{{PIPELINE_ROOT}}/agents/**/*.md` + `{{PIPELINE_ROOT}}/agents/*.md` — global user agents
...
- **User agents** have no prefix. Read the corresponding markdown file from `{{PIPELINE_ROOT}}/agents/` or `./agents/` to extract the name and description.
