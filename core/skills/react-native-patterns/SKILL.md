...
Libraries named below (NativeWind, Zustand/Jotai, TanStack Query) are common, well-established options shown for illustration — the patterns matter more than the specific package, and any equivalent works. Zod is used for validation to stay consistent with this pipeline existing `typescript/` rules.
...
- `test-first`, `e2e-testing` — testing process (use Jest + React Native Testing Library, Maestro/Detox for RN).
