...
### Phase 3: Evaluate
Run the definition's graders and record pass@k
...
## Evaluating the Pipeline Itself

The same discipline applies to the agents, hooks, and rules you ship. `acp eval`
runs behavioural evaluations of the pipeline against scratch projects:

```sh
acp eval list                 # what is covered
acp eval run                  # every evaluation; exit 1 on any failure
acp eval new <name>           # scaffold harness/evals/<name>/{eval.sh,README.md}
```

Each evaluation installs into a fresh directory, drives a hook or a driver, and
asserts on what happened. A change to an agent or a rule that could regress
silently gets an evaluation before it merges, the same way code gets a test.
