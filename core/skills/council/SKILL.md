...
| --- | --- |
| Verifying whether output is correct | `code-reviewer` |
...
Do **not** write ad-hoc notes to `{{PIPELINE_ROOT}}/notes` or other shadow paths from this skill.

If the council materially changes the recommendation:
- use `knowledge-ops` to store the lesson in the right durable location
- or use `/next-session` if the outcome belongs in session memory
...
- `code-reviewer` — adversarial verification
...
```text
Should we ship the new billing system as a beta now, or hold until the admin UI for refunds is complete?
```
