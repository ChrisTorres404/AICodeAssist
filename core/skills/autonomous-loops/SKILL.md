...
| [Persistent Session](#2-persistent-session-loop) | Low | Interactive persistent sessions |
...
| [Work-Order DAG](#6-work-order-dag-orchestration) | High | Large features, multi-unit parallel work with merge queue |
...
**A conversation that survives restarts.** The session is the loop: each turn resumes the same context, so the agent carries what it learned instead of starting cold.

```bash
# Start a named session and keep resuming it
claude --session-id my-project "Start work order 0412; read its SPEC first."
claude --resume my-project "Continue: the checklist says step 3 is next."

# Or simply pick up the most recent conversation in this directory
claude --continue
```

### How It Works

1. The conversation is the state; every turn sees everything before it
2. Between turns, the work order's CHECKLIST and `wo note` entries hold the durable record, so a fresh session can rebuild context from the repository even if the conversation is lost
3. The `session-handoff` skill writes the handoff at the end of each run and `session-orient` reads the in-flight work orders at the start of the next
4. The pre-compaction hook writes an orientation note before context is compacted

### When a Persistent Session vs Sequential Pipeline

| Use Case | Persistent Session | Sequential Pipeline |
|----------|--------------------|---------------------|
...
**A production-grade shell script** that runs Claude Code in a continuous loop, creating PRs, waiting for CI, and merging automatically..
...
### When to Use the work-order DAG runner vs Simpler Patterns

| Signal | Use the work-order DAG runner | Use Simpler Pattern |
|--------|--------------|-------------------|
...
| Quick iteration on one thing | No | Yes (persistent session or pipeline) |
...
├─ Yes → Sequential Pipeline or Persistent Session
└─ No → Is there a written spec/RFC?
         ├─ Yes → Do you need parallel implementation?
         │        ├─ Yes → the work-order DAG runner (DAG orchestration)
...
3. **Any loop + Verification** — Use the `/verify` command or the `readiness-check` skill as a gate before commits.

4. **the work-order DAG runner's tiered approach in simpler loops** — Even in a sequential pipeline, you can route simple tasks to Haiku and complex tasks to Opus:
   ```bash
