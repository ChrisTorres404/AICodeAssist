...
| Code review | the `code-reviewer` agent or the `code-expert` |
| Structured adversarial challenge | `code-reviewer` |
...
- "Plan the work" — use the `planner` role for an implementation plan, or the `epic-*` commands
  (`/plan`, then `wo publish`) for issue-backed breakdown
...
- `/prd` (command) — product requirements document before the team session
- `/plan` + `wo publish` (commands) — break the outcome into issue-backed work
