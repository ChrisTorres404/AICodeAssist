...
`Stop` events do not require a `matcher` field (it is optional for `Stop`, `Notification`, `UserPromptSubmit`, and `SubagentStop` as `acp lint` validates). If omitted, the hook object only needs `hooks` and metadata such as `description`.
...
echo "Your agent response here" | python3 .claude/skills/agent-self-evaluation/scripts/evaluate.py

# From files
python3 .claude/skills/agent-self-evaluation/scripts/evaluate.py --task task.txt --output response.txt
```
