...
class the databaseMarketRepository implements MarketRepository {
  async findAll(filters?: MarketFilters): Promise<Market[]> {
    let query = the database.from('markets').select('*')
...
async function createMarketWithPosition(
  ...
  const { data, error } = await the database.rpc('create_market_with_position', {
