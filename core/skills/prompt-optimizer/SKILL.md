...
description: "Analyze raw prompts, identify intent and gaps, match pipeline components (skills/commands/agents/hooks), and output a ready-to-paste optimized prompt. Advisory role only — never executes the task itself. TRIGGER when: user says 'optimize prompt', 'improve my prompt', 'how to write a prompt for', 'help me prompt', 'rewrite this prompt', or explicitly asks to enhance prompt quality. Also triggers on Chinese equivalents: '优化prompt', '改进prompt', '怎么写prompt', '帮我优化这个指令'. DO NOT TRIGGER when: user wants the task executed directly, or says 'just do it' / '直接做'. DO NOT TRIGGER when user says '优化代码', '优化性能', 'optimize performance', 'optimize this code' — those are refactoring/performance tasks, not prompt optimization. Use when the task calls for prompt optimizer."
---

# Prompt Optimizer

Analyze a draft prompt, critique it, match it to this pipeline's components,
...
- User says "how should I use this pipeline for..."
- User asks for the `prompt-optimizer` skill

### Do Not Use When

- User wants the task done directly (just execute it)
- User says "优化代码", "优化性能", "optimize this code", "optimize performance" — these are refactoring tasks, not prompt optimization
- User is asking about this pipeline configuration (use `project-onboarding` instead)
...
### Phase 3: Pipeline Component Matching

Map intent + scope + tech stack (from Phase 0) to specific pipeline components.

#### By Intent Type

| Intent | Commands | Skills | Agents |
|--------|----------|--------|--------|
| New Feature | /plan, /tdd, /code-review, /verify | test-first, verification | planner, tdd-guide, code-reviewer |
| Bug Fix | /tdd, /build-fix, /verify | test-first | tdd-guide, build-error-resolver |
| Refactor | /refactor-clean, /code-review, /verify | verification | refactor-cleaner, code-reviewer |
| Research | /plan | search-first, iterative-retrieval | — |
| Testing | /tdd, /e2e, /test-coverage | test-first, e2e-testing | tdd-guide, e2e-runner |
...
- Use /next-session and `claude --continue` to preserve context between sessions
...
### Section 2: Recommended Pipeline Components

| Type | Component | Purpose |
|------|-----------|---------|
| Command | /plan | Plan architecture before coding |
| Skill | test-first | TDD methodology guidance |
...
A compact version for experienced users of this pipeline. Vary by intent type:
...
- "Optimize this prompt for this pipeline"
- "Rewrite this prompt so Claude Code uses the right commands"
- "帮我优化这个指令"
- "How should I prompt this pipeline for this task?"
...
Use /next-session between phases; resume with `claude --continue`.
...
|-----------|------------------|
| `project-onboarding` | User hasn't set up this pipeline yet |
...
| `/prd` then `/plan` | Epic-scope work that should become work orders |
