...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "matcher": "Edit",
        "hooks": [{ "type": "command", "command": "node {{PIPELINE_ROOT}}/scripts/hooks/suggest-compact.js" }]
      },
      {
        "matcher": "Write",
        "hooks": [{ "type": "command", "command": "node {{PIPELINE_ROOT}}/scripts/hooks/suggest-compact.js" }]
...
- `ACP_CONTEXT_WINDOW_TOKENS` — Explicit context-window size, in tokens, overriding auto-detection. Set this for large-window models whose reported id lacks a `[1m]` marker (e.g. 400k Opus 4.x, or a new 1M-window model family) so the threshold scales to the real window instead of defaulting to 200k and overstating context usage.
- `CLAUDE_CODE_AUTO_COMPACT_WINDOW` — Claude Code's native window-size override, in tokens; honored as a fallback when `ACP_CONTEXT_WINDOW_TOKENS` is unset.
...
| Memory files (`{{PIPELINE_ROOT}}/memory/`) | Multi-step conversation context |
...
|---------|-------|-----------|
| "test", "tdd", "coverage" | test-first | User mentions testing |
...
- Same rules in both `{{PIPELINE_ROOT}}/rules/` and project `.claude/rules/`
...
- `session-handoff` skill — captures what the next session needs before this one ends
