...
- Skills: [react-performance](../react-performance/SKILL.md) for the performance ruleset, [frontend-patterns](../frontend-patterns/SKILL.md) for cross-framework UI concerns, [accessibility](../accessibility/SKILL.md), [angular-patterns](../angular-patterns/SKILL.md) for framework comparison
- Agents: `react-reviewer` for code review, `react-build-resolver` for build/bundler errors
- Commands: the `react-expert`, the `react-expert`, the `react-expert`
