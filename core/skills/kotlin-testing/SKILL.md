...
7. **Check coverage** — Run `./gradlew koverHtmlReport` and verify executed behavioural evidence (see the testing rule)
...
- **TDD walkthrough** — Full RED/GREEN/REFACTOR cycle with EmailValidator in [TDD Workflow for Kotlin](#test-first-for-kotlin)
...
kover {
    reports {
        ...
        verify {
            rule {
                minBound(80) // Fail build below executed behavioural evidence (see the testing rule)
            }
        }
    }
}
