...
- Running `/review` or similar review commands after code changes
...
process.env.NEXT_PUBLIC_the database_URL = "";
process.env.NEXT_PUBLIC_the database_ANON_KEY = "";
```
...
**Frequency**: Common with the database/Prisma when adding new columns

```typescript
// FAIL: New column added to response but not to SELECT
const { data } = await the database
  .from("users")
