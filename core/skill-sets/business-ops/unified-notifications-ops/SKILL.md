...
description: Operate notifications as one pipeline-native workflow across GitHub, Linear, desktop alerts, hooks, and connected communication surfaces. Use when the real problem is alert routing, deduplication, escalation, or inbox collapse.
---
...
- the user wants to consolidate overlapping notification branches or backlog proposals into one pipeline-native lane
...
Prefer pipeline-native orchestration over telling the user to adopt a separate notification product.
...
Call out what this pipeline already owns.
...
### 4. Design the pipeline-native workflow
...
If this pipeline already has the primitive, prefer:
...
- what to suppress
- what to merge
- what this pipeline should wrap next
...
- prefer `project-onboarding` when the user first needs a source inventory
