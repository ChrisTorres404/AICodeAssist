...
Nothing here is installed unless you ask for it: set `SKILL_SETS` in
`pipeline.config.sh` (space-separated), or pass `--skill-set <name>` to
`new-project`. The `full` install profile installs every set.
...
set by creating a directory here; `acp lint` validates it like any skill.
