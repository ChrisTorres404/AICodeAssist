...
description: Fork any project for open-sourcing. Copies files, strips secrets and credentials (20+ patterns), replaces internal references with placeholders, generates .env.example, and cleans git history. First stage of the opensource-pipeline skill. Use when the task calls for a open-source forker.
model: sonnet
...
Run release-sanitizer to verify sanitization is complete.
```
...
- "Next step: run release-sanitizer"
