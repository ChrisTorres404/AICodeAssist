...
### Review questions added after real incidents
- Is any security code commented out?
- Does this query include the tenant filter?
- When cookies are set, are the same tokens absent from the body?
- Does this change duplicate a resolution path (permissions, claims, tenant, theme) that already has a single owner?
- When code was extracted or cloned, was the original deleted, and did the guard and decorator envelope come along?
- Do 4xx or 5xx responses add `Set-Cookie` headers, and are they safe through a proxy?
- Does time arithmetic name its units?
- Is this querying the event or the current state?
- Do raw SQL column names match the schema, not the entity?
- Are there hard-coded UUIDs, tenant ids, or colour classes?
- Are all documented code paths (comments describing behaviour) actually implemented, with a test per branch?
