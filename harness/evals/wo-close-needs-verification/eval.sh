...
[ "$(grep -c "^## WO-$n " .aicodepipeline/packs/proj/CATALOG.md)" -eq 1 ] || { echo "re-promote duplicated the catalog entry"; exit 1; }
grep -q "Suites must clean up" .aicodepipeline/packs/proj/CATALOG.md || { echo "catalog entry lacks the pitfalls"; exit 1; }
[ -d ".aicodepipeline/packs/proj/workorders/WO-$n-Eval-order" ] || { echo "promote nested the pack path wrongly:"; find .aicodepipeline/packs -maxdepth 3 -type d; exit 1; }
