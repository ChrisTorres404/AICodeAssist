...
# a step that is not done fails its suite and names the fix
...
# the knowledge base step: fails without one, passes with a bound one
rc=0; $W verify 6 --run Workspace/Testing/suites/intake-06-*.sh >/dev/null 2>&1 || rc=$?
[ "$rc" -ne 0 ] || { echo "the knowledge-base step passed with no knowledge base"; exit 1; }
mkdir -p Workspace/Docs/KnowledgeBase/profiles src
printf 'export const x = 1;\n' > src/x.ts
printf '# Repository analysis\n\nOne feature.\n' > Workspace/Docs/KnowledgeBase/analysis.md
printf '# X — Feature Profile\n\nX is a constant. <!-- SOURCE: src/x.ts:L1 -->\n' > Workspace/Docs/KnowledgeBase/profiles/x.md
$A kb bind >/dev/null 2>&1 || { echo "kb bind failed"; exit 1; }
git add -A >/dev/null 2>&1; git commit -q -m "chore: knowledge base" >/dev/null 2>&1
$W verify 6 --run Workspace/Testing/suites/intake-06-*.sh >/dev/null 2>&1 || { echo "the knowledge-base step failed with a bound knowledge base:"; tail -6 Workspace/Docs/WorkOrders/WO-0006-*/WO-0006-VERIFICATION-*.log; exit 1; }
